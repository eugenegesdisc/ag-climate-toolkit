...
if [ "${upper_crop}" = "PEANUTS" ] || \
    ...
    the_unit_desc_con="unit_desc;;GALLONS / TAP"
elif [ "${upper_crop}" = "HAY & HAYLAGE" ]; then
    the_unit_desc_con="unit_desc;;TONS / ACRE, DRY BASIS"
fi
...
echo "survey-${crop}-yield-${lower_state_name}"

python "${py_src}" "nass_quickstats" \
    --operation ${the_operation} \
    --api-key "${api_key}" \
    --parameter "${the_parameter}" \
    --output "${output_dir}/survey_${lower_state_code}_${lower_crop}_yield.parquet" \
    --output-columns "year;Value" \
    --output-column-names "year;${lower_crop}_yield" \
    --search-conditions \
                "source_desc;;SURVEY" \
                "sector_desc;;CROPS" \
