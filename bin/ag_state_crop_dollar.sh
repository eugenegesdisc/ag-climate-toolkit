...
echo "survey-${crop}-dollar-${lower_state_name}"

python "${py_src}" "nass_quickstats" \
    --operation ${the_operation} \
    --api-key "${api_key}" \
    --parameter "${the_parameter}" \
    --output "${output_dir}/survey_${lower_state_code}_${lower_crop}_dollar.parquet" \
    --output-columns "year;Value" \
    --output-column-names "year;${lower_crop}_dollar" \
    --search-conditions \
                "source_desc;;SURVEY" \
                "sector_desc;;CROPS" \
