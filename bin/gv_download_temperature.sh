...
: <<'COMMENT'
        {
            ...
            "args": [
                ...
                "--plot-variable","2-meter Air Temperature monthly mean M2SMNXSLV",
                "--rename-column","temperature",
                "--rename-column-index","1",
                "--save-to-csv-file","~/output/monthly_2m_air_temperature.csv",
                "--save-to-csv-file-metadata",
                "--csv-skip-signature","time, mean_",
                "--save-to-parquet-file","~/output/monthly_2m_air_temperature.parquet"
            ]
        }
COMMENT
...
python "${py_src}" \
    ...
    --rename-column "temperature" \
    --rename-column-index "1" \
    --save-to-csv-file "${output_dir}/${lower_state_code}_monthly_temperature.csv" \
    --save-to-csv-file-metadata \
    --csv-skip-signature "time, mean_" \
    --save-to-parquet-file "${output_dir}/${lower_state_code}_monthly_temperature.parquet"
