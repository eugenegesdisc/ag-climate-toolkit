...
if [ $# -lt 6 ]; then
    ...
    echo "Usage: bash $0 <python-src-dir> <api-key> <output-dir> <state-2-letter> <state-full-name> <crop> {start_year} {operation}  {parameter-to-desc}"
...
the_unit_desc_con="unit_desc;;ACRES"
if [ "${upper_crop}" = "MAPLE SYRUP" ]; then
    the_unit_desc_con="unit_desc;;ACRES WHERE TAPS SET"
fi
...
#    [ "${upper_crop}" = "CANOLA" ] || \
#    [ "${upper_crop}" = "LENTILS" ] || \
#    [ "${upper_crop}" = "COTTON" ]; then
#    the_unit_desc_con="unit_desc;;LB / ACRE"
#elif [ "${upper_crop}" = "HAY" ]; then
#    the_unit_desc_con="unit_desc;;TONS / ACRE"
#fi

the_statisiccat_desc_cond="statisticcat_desc;;AREA PLANTED"
if [ "${upper_crop}" = "HAY" ] || \
    [ "${upper_crop}" = "TOBACCO" ] || \
    [ "${upper_crop}" = "HOPS" ] || \
    [ "${upper_crop}" = "HAY & HAYLAGE" ] || \
    [ "${upper_crop}" = "SUGARCANE" ]; then
    the_statisiccat_desc_cond="statisticcat_desc;;AREA HARVESTED"
elif [ "${upper_crop}" = "MAPLE SYRUP" ]; then
    the_statisiccat_desc_cond="statisticcat_desc;;AREA"
fi
...
echo "survey-${crop}-acres-${lower_state_name}"
echo "the_statisiccat_desc_cond=${the_statisiccat_desc_cond}"
echo "the_util_practice_desc_con=${the_util_practice_desc_con}"

python "${py_src}" "nass_quickstats" \
    --operation ${the_operation} \
    --api-key "${api_key}" \
    --parameter "${the_parameter}" \
    --output "${output_dir}/survey_${lower_state_code}_${lower_crop}_acres.parquet" \
    --output-columns "year;Value" \
    --output-column-names "year;${lower_crop}_acres" \
    --search-conditions \
                "source_desc;;SURVEY" \
                "sector_desc;;CROPS" \
